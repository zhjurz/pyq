...
数据库表结构由 `ensureReady()` 在首次请求时自动通过 `sequelize.sync()` 创建（只建缺失表，不改已存在表的结构）。**管理员账号需要手动 seed 一次**（和 VPS 部署一样）：
...
6. **MusicFree 音源播放采用“后端解析、浏览器直连”模式**：后端仅执行插件并返回短期播放 URL，音频字节不经过 Vercel 函数。插件若要求 Referer、Cookie、User-Agent 等自定义播放请求头，会在严格直连模式下返回不支持提示；此类音源不会回退到 Vercel 音频代理。
...
**首次部署恢复方式**：该错误会使对应的 `CREATE TABLE` 语句整体失败，不会留下半张 `music_sources` 表。部署修复后的代码后，直接再请求一次 `/api/health`（或任意 API）即可：`sequelize.sync()` 会保留之前已成功创建的表，并创建此前失败或尚未创建的表。**不要为这个首次建表错误删除已有业务表。**
...
SHOW CREATE TABLE music_sources;
